...
public class ConeMqttService {
	 ...
	 * @param id
	 */
	public LoginAck LoginOnly(String fsuId, String uniqueCode) throws Exception{
		ConeFsu fsu = interfaceFsuService.queryFsuById(uniqueCode, fsuId);// 数据库中获取FSU信息
		String ip = fsu.getIp();// FSU IP
		int port = fsu.getPort();// FSU 端口
		String name = fsu.getUsername();// FSU 用户名
	 ...
	 * @param id
	 */
	public LogoutAck Logout(String fsuId, String uniqueCode)  throws Exception{
		ConeFsu fsu = interfaceFsuService.queryFsuById(uniqueCode, fsuId);// 数据库中获取FSU信息
		String ip = fsu.getIp();// FSU IP
		int port = fsu.getPort();// FSU 端口
		String key = ip + "#" + port;
		...
		LogoutAck out = config.getInterfaceFactory().LoginOut();//用户登出接口
	 ...
	 * @param id
	 */
	public boolean synchronousProperty(String fsuId, String uniqueCode)   throws Exception{
		ConeFsu fsu = interfaceFsuService.queryFsuById(uniqueCode, fsuId);// 数据库中获取FSU信息
		String ip = fsu.getIp();// FSU IP
		int port = fsu.getPort();// FSU 端口
		String key = ip + "#" + port;
	 ...
	 */
	public boolean getAccessDate(String deviceId, String uniqueCode)   throws Exception {
		...
		CInterfaceConfig config = ConeStaticMessageMap.configMap.get(key);
		boolean flag = setFsuAccessMode(config.getInterfaceFactory(), uniqueCode, fsuId, deviceInfo.getC1_id());
		return flag;
	}
	 ...
	 */
	public SetPointAck setPoint(String uniqueCode, String signalId, Double value, String fsuId)   throws Exception {
		...
		Body data = interfaceSignalService.getBodyCone(uniqueCode, signal);
	 ...
	 * @param fsuId
	 */
	public ReqSetPropertyAck setProperty(String uniqueCode, String signalId, double threshold, String fsuId)   throws Exception {
		ConeSignal signal = interfaceSignalService.querySignalById(uniqueCode, signalId);
	 ...
	 * @param userName
	 */
	public void verifyAlarm(String uniqueCode, String fsuId, String alarmId, String userName)  throws Exception {
	...
	private boolean getFsuProperty(InterfaceFactory factory, String uniqueCode, String fsuId) {
		try {
			...
			if (c1_fsuId == 0) {
				throw new Exception("当前无设备ID --> get Node is Exception !");
			}
	 ...
	 * @param c1_fsuId
	 */
	private boolean setFsuAccessMode(InterfaceFactory factory, String uniqueCode, String fsuId, int device_c1Id) {
		try {
